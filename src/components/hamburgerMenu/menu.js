...
import { CUSTOMER_DASHBOARD, ORDER_DASHBOARD, FULFILLMENT_DASHBOARD, PRODUCT_DASHBOARD, USER_DASHBOARD, CUSTOMER_DETAILS } from '../../constants/routesConstant';
import { Link } from "react-router-dom";
import Tabs from '@material-ui/core/Tabs';
import CloseIcon from '@material-ui/icons/Close';
import {
  DashboardNav, DashboardNavSelected,
  FulfillmentNav, FulfillmentNavSelected,
  CustomerNav, CustomerNavSelected,
  ProductNav, ProductNavSelected,
  UserNav, UserNavSelected
} from '../../constants/images/sideNavIcon';
...
const Menu = ({ open, setOpen }) => {
  ...
  useEffect(() => {
    ...
    switch (true) {
      case location.pathname.includes('order'): setNavSelected(0);
        break;
      case location.pathname.includes('fulfillment'): setNavSelected(1);
        break;
      case location.pathname.includes('customer'): setNavSelected(2);
        break;
      case location.pathname.includes('product'): setNavSelected(3);
        break;
      case location.pathname.includes('user'): setNavSelected(4);
        break;
      default: break;
    }
  }, [location]);
  ...
  return (
    <nav className={`${open ? 'hamburgerMenu' : " hamburgerMenu hamburgerMenuShortcut"} ${showNavbar ? '' : 'hideNavbar'}`} style={{ transform: open ? 'translateX(0)' : 'translateX(0%)' }}>
      ...
      >
        ...
        <Tab label={open ? 'Fulfillment' : ''} icon={navSelected === 1 ? FulfillmentNavSelected : FulfillmentNav} value={1} component={Link} to={FULFILLMENT_DASHBOARD} />
